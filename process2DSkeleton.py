...
def draw_skeleton(joints_list,show=0,save=0,autoscale=0,outdir="",outname=""):
  ...
  for jj in range(0,len(joints_list)):
    ...
    for num,joint in joints.iteritems():
      joint[1] = -joint[1]
      ax.scatter(joint[0],joint[1],color=colors[jj])
      if autoscale:
        X.append(joint[0])
        Y.append(joint[1])
    ...
    for connection in connectivity:
      ...
      ax.plot([joints[f][0],joints[t][0]],[joints[f][1],joints[t][1]],marker = '.')
    ax.plot([joints[8][0],joints[4][0]],[joints[8][1],joints[4][1]],color='black')
...
def normalize_skeleton(jinfo,anchor=None,norm_dist=0,right_to_left=None,spine_to_top=None):
  if np.logical_and((anchor is None), (norm_dist == 0)):
  ...
  if right_to_left is None:
    right_to_left = np.array([norm_joints[8][0],norm_joints[8][1]]) - np.array([norm_joints[4][0],norm_joints[4][1]])
  ...
  #pdb.set_trace()
  #Rx = np.matrix(x, -y; y, x)   #ROTATION MATRIX
  ...
  # new_right_to_left = np.array([rotated_and_norm[8][0],rotated_and_norm[8][1]]) - np.array([rotated_and_norm[4][0],rotated_and_norm[4][1]])
  

  if spine_to_top is None:
    spine_to_top = np.array([norm_joints[0][0],norm_joints[0][1]]) - np.array([norm_joints[1][0],norm_joints[1][1]])
  ...
  Ry = np.array([[spine_to_top[1], np.negative(spine_to_top[0])], [spine_to_top[0], spine_to_top[1]]])


  rotated_and_norm2 = {}
  for jnum, joint in norm_joints.iteritems():
    #turn_to_x = np.dot(Rx,joint)
    turn_to_y = np.dot(Ry,joint)
    ...
    rotated_and_norm2[jnum] = normed_final_vec

  return rotated_and_norm2,anchor,norm_dist,right_to_left,spine_to_top


def vids_with_missing_skeletons():
  f = open('/home/siqinli/dataset/nturgbd/samples_with_missing_skeletons.txt','r')
  bad_files = []
  for line in f:
...
def find_nan_data():
  f = open('/home/siqinli/dataset/nturgbd/nan_file.txt','r')
  bad_files = []
  for line in f:
...
def generate_data(argv):
  missing_files = vids_with_missing_skeletons()
  nan_files = find_nan_data()
  bad_files = missing_files + nan_files

  skeleton_dir_root = "/home/siqinli/dataset/nturgbd/nturgb+d_skeletons"
  skeleton_files = os.listdir(skeleton_dir_root)
  data_out_dir = '/home/siqinli/dataset/data_out/cross_subject_2d_norm/'
  ...
  for file_name in skeleton_files:
      ...
      if body_count > 2:
        ...
        for bind, body in binfo.iteritems():
          for jind, joint in body.iteritems():
            ...
            sample_ind += 1
            # sample[sample_ind] =   1 #z
            # sample_ind += 1
        feature[n] = sample

        # print binfo
        # if np.isnan(np.sum(feature[n])):
        #   print unnorm_binfo
        #   print feature[n]
        # draw_skeleton(binfo,show=1,save=0,autoscale=1,outdir="",outname="")
    ...
    ## INTERPOLATION
    # lst = [np.isnan(np.sum(item)) for item in feature]
    # lst = np.sum(feature)
    # # print len(lst)
    # if np.isnan(lst):
    #   # print file_name
    #   nan_file.write(file_name+"\n")
